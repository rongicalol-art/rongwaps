...
- Chosen:
  ...
  - Batch tooling generalized for the rollout: `prepareBookOneEvaluationBatch.ts` accepts `--batch <stem> --characters "字 字 …"` (defaults to batch-47), and `applyBatchDrafts.ts` / `applyCuratedDraftsToPack.ts` accept `--batch <stem>`, so rollout batches get their own artifacts without touching the frozen batch-47 files. Rollout run order matters: `curate:freeze` → `prepare` → `applyMeaningAudit --apply --characters …` (reviewer meaning overrides are layer-patched into `book-1-plans.json`, and a fresh `prepare` resets them) → `prepare:batch -- …` → draft/validate → `apply:batch-drafts` → `apply:pack` → `export`.
