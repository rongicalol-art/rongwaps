...
- **Scope**:
  ...
  - Memory-hook content audits (`tests/acceptance/memory_hooks.test.ts`): pack manifest hash/count, an emphasis run on every hook, word hooks naming their characters in order, character hooks mentioning parts in breakdown order (against `tests/fixtures/memory-hook-decomposition-trees.json`, regenerated by `memory-hooks:snapshot:order`), and retired phrasings staying gone.
