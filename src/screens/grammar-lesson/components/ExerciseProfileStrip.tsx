  ...
  return (
    <div className="grid grid-cols-2 gap-3 xl:grid-cols-4" aria-label="Exercise profiles">
      {profiles.map((profile) => (
        <article
          key={profile.id}
          className="flex min-h-[104px] items-center gap-2 rounded-feature border border-ui-divider bg-ui-surface p-3 text-left sm:min-h-[116px] sm:gap-4 sm:p-4"
        >
          <RongWapsCharacterPortrait
            character={profile.avatar}
            label={`${profile.surname}${profile.givenName}`}
            className="h-14 w-14 shrink-0 rounded-control sm:h-[72px] sm:w-[72px]"
          />
          <div className="min-w-0">
            <p className="font-chinese text-sm font-black text-ui-ink-strong sm:text-lg">
              {profile.surname}{profile.givenName}
            </p>
            <p className="truncate text-xs font-bold text-ui-muted-strong sm:text-sm">{profile.fullNamePinyin}</p>
