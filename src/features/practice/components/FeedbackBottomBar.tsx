...
export interface FeedbackBottomBarProps {
  ...
  onSkip?: () => void;
  onBreakdown?: () => void;
  /** Whether the inline breakdown panel is expanded (toggled by `onBreakdown`). */
  breakdownOpen?: boolean;
  /** Inline breakdown content rendered above the action row when expanded. */
  breakdownPanel?: ReactNode;
  onRetry?: () => void;
  activeBook?: {
    accent: string;
    bg: string;
          ...
          <div className="mx-auto flex w-full max-w-2xl flex-col gap-4 px-4 pb-6 pt-4 md:gap-5 md:pb-10 md:pt-6">
            ...
            <AnimatePresence initial={false}>
              {breakdownOpen && breakdownPanel && (
                ...
                >
                  <div className="px-1">{breakdownPanel}</div>
                </motion.div>
              )}
            </AnimatePresence>
